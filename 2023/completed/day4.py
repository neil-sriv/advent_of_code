...
--- Day 4: Scratchcards ---
   ...
   visit the gardener. I got all these [15]scratchcards as a gift, but I
...
def part1(data: list[str]) -> int:
    ...
    for line in data:
        ...
        winning, yours = [
            set([int(val.strip()) for val in spl.split()]) for spl in l.split(" | ")
        ]
        cards = winning.intersection(yours)
        total += 2 ** (len(cards) - 1) if len(cards) > 0 else 0
    return total
...
def part2(data: list[str]) -> int:
    ...
    for i, line in enumerate(data):
        ...
        cards.update(
            {i+1: {"winning_num": len(winning.intersection(yours)), "copies": 0}}
        )

    for i in range(1, 1+len(data)):
        copies = cards[i]["copies"] + 1
        for j in range(cards[i]["winning_num"]):
            cards[i + j + 1]["copies"] += copies
    return sum([cards[i]["copies"]+1 for i in range(1, 1+len(data))])
