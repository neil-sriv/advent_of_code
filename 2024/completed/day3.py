...
--- Day 3: Mull It Over ---
   ...
   though," says the mildly flustered shopkeeper at the [15]North Pole
...
def part1(data: list[str]) -> int:
    regexp = re.findall(r"mul\((\d{1,3}),(\d{1,3})\)", " ".join(data))
    total = 0
    for one, two in regexp:
...
def part2(data: list[str]) -> int:
    string = " ".join(data)
    strings = re.split(r"(do\(\))|(don't\(\))", string)
    print(strings)
    total = 0
    do = True
    for i in range(0, len(strings)):
        ...
        if strings[i] == "do()":
            do = True
        elif strings[i] == "don't()":
            do = False
        if not do:
            continue
        s = strings[i]
        regexp = re.findall(r"mul\((\d{1,3}),(\d{1,3})\)", s)
        for one, two in regexp:
            total += int(one) * int(two)
    return total
